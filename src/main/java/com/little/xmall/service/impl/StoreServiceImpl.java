...
public class StoreServiceImpl extends ServiceImpl<StoreInfoMapper, StoreInfo> implements StoreService {
    ...
    public Response<Map<String, Object>> changePassword(StorePassword password) {
        LambdaQueryWrapper<StoreInfo> storeInfoQueryWrapper = new LambdaQueryWrapper<>();
        storeInfoQueryWrapper.eq(StoreInfo::getStore_id, password.getStore_id());
        ...
        if (!Objects.equals(storeInfo.getStore_id(), password.getStore_id())) {
            ...
            if (bCryptPasswordEncoder.matches(password.getOld_password(), storeInfo.getPassword())
                    || password.getOld_password().equals(storeInfo.getPassword())) {
