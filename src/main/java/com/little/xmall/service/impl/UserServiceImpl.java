...
public class UserServiceImpl extends ServiceImpl<UserInfoMapper, UserInfo> implements UserService {
    ...
    public Response<Map<String, Object>> changePassword(UserPassword password) {
        ...
        userInfoQueryWrapper.eq(UserInfo::getUser_id, password.getUser_id());
        UserInfo userInfo = userInfoMapper.selectOne(userInfoQueryWrapper);
        if (!Objects.equals(userInfo.getUser_id(), password.getUser_id())) {
            return Response.error(ResponseCode.FAIL, null);
        } else {
            if (bCryptPasswordEncoder.matches(password.getOld_password(), userInfo.getPassword())
                    || password.getOld_password().equals(userInfo.getPassword())) {
    ...
    @Override
    public Response<List<Map<String, Object>>> getRecord(Integer user_id) {
        LambdaQueryWrapper<HistoryInfo> historyInfoQueryWrapper = new LambdaQueryWrapper<>();
        List<HistoryInfo> list = historyInfoMapper.selectList(historyInfoQueryWrapper.eq(HistoryInfo::getUser_id, user_id));
        for (HistoryInfo historyInfo : list) {
            Integer goods_id = historyInfo.getGoods_id();
            historyInfo.setGoodsInfo(MapUtil.fromMap(goodsServiceImpl.getGoods(goods_id).getData(), GoodsInfo.class));
        }
        return Response.success(ResponseCode.SUCCESS, MapUtil.getMapList(list));
    }

    @Override
    public Response<String> deleteRecord(Integer user_id, Integer goods_id) {
        LambdaQueryWrapper<HistoryInfo> historyInfoQueryWrapper = new LambdaQueryWrapper<>();
        historyInfoQueryWrapper.eq(HistoryInfo::getGoods_id, goods_id).eq(HistoryInfo::getUser_id, user_id);
        HistoryInfo historyInfo = historyInfoMapper.selectOne(historyInfoQueryWrapper);
        historyInfo.setIs_delete(true);
